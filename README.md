...
## КАК ЗАПУСТИТЬ ТРЕКИНГ
Для запуска трекинга вам понадобится взять ваш гитхаб токен, к которому присоединена возможность чтения репы
$env:GITHUB_TOKEN="ВАШ_ТОКЕН", после чего запустить скраппер-бд
